...
from app.extractors import (
    ...
    _select_customer_ice,
    _select_supplier_tax_id,
    is_valid_ice,
...
@pytest.mark.parametrize(
    "entries, expected",
    [
        ([("ICE", "001234567000089")], "001234567000089"),
        (
            [
                ("ICE", "001234567000089"),
                ("ICE", "001234567000089"),
...
def test_select_customer_ice(entries, expected):
    assert _select_customer_ice(entries) == expected
...
def test_select_supplier_tax_id(entries, expected):
    assert _select_supplier_tax_id(entries) == expected
